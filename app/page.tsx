...
export default async function MarketingPage() {
  ...
  const primaryHref = isAuthenticated
    ? "/chat"
    : `/api/auth/login?returnTo=${encodeURIComponent("/chat")}`;
  ...
  return (
    <div className="space-y-24 bg-gradient-to-b from-white via-white to-slate-50 pb-24">
      <section className="mx-auto flex max-w-5xl flex-col items-center gap-6 px-6 pt-16 text-center">
        <span className="rounded-full border border-black/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          Now with secure Auth0 sign-in
        </span>
        ...
        <div className="w-full rounded-3xl border border-dashed border-black/10 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-12 text-left text-white shadow-2xl">
          ...
          <p className="mt-3 max-w-xl text-white/80">
            Authenticated teams get routed into the existing chat workspace you built. Every conversation remains private,
            auditable, and tied back to the authenticated user.
          </p>
          <div className="mt-6 h-48 rounded-2xl border border-white/20 bg-black/40" />
        </div>
      </section>
