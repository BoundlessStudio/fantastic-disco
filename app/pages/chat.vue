...
> = {
  "core-researcher": {
    title: "Core Research Agent",
    tagline: "Research-grade answers with Mem0 recall and integrated web tools.",
  },
  "ops-desk": {
    title: "Ops Desk Agent",
    tagline: "Monitors inbound issues, drafts updates, and closes feedback loops.",
  },
  "executive-brief": {
    title: "Executive Briefing Agent",
    tagline: "Turns unstructured updates into crisp leadership-ready briefings.",
  },
};
...
const activeAgentId = computed(
  () => (route.query.agent as string) ?? "core-researcher",
);
const activeAgent = computed(
  () => agentDirectory[activeAgentId.value] ?? agentDirectory["core-researcher"],
);
const agentDisplayName = computed(() => activeAgent.value.title);
const agentTagline = computed(() => activeAgent.value.tagline);
const userName = computed(
  () => auth0?.user.value?.name ?? "Authenticated teammate",
);
...
<template>
  <div class="min-h-screen bg-background">
    <header class="border-b bg-background/80 backdrop-blur">
      <div
        class="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-4 md:flex-row md:items-center md:justify-between"
      >
        <div>
          <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Now chatting with
          </p>
          <p class="text-2xl font-semibold">{{ agentDisplayName }}</p>
    ...
    <main class="relative min-h-[calc(100vh-4rem)] bg-background">
      <div
        class="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-7xl flex-col px-4 py-10 md:px-8"
      >
